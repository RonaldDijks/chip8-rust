...
impl Gui {
    ...
    pub fn render(
        ...
        let mut cpu_info_open = false;

        ui.main_menu_bar(|| {
            ui.menu("Views", || {
                cpu_info_open = imgui::MenuItem::new("CPU Info").build(&ui);
            });
            ui.menu("Help", || {
        ...
        if self.cpu_info_open {
            Window::new("CPU Info").build(&ui, || {
                ui.text(format!("PC: {}", cpu.pc));
                ui.text(format!("I: {}", cpu.index))
            });
        }
